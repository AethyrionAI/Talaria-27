# FoundationModels beta-4 swiftinterface transcription (`fmf.md`)

**Vendored verbatim** from `rudrankriyam/Foundation-Models-Framework-Lab` @ `0c8fda2`
(fetched 2026-07-28, MIT — see THIRD_PARTY_LICENSES.md "Foundation-Models-Framework-Lab").
A 9,737-line transcription of the Xcode 27 beta FoundationModels Swift interface
INCLUDING doc comments, which the raw on-disk `.swiftinterface` strips. Produced
upstream by grepping the installed SDK (their `os27-updater` skill documents the method),
versioned beta1→beta4. Kept here for grep-reference during the #200-family lanes —
notably the `ToolCallingMode.required` loop-semantics doc comment (see OPEN_ITEMS
#200 addendum 2). Trust the local SDK interface over this file if they ever disagree.
