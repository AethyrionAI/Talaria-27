...
- **Xcode-rc** (`/Applications/Xcode-rc.app`, Xcode 27.0 build **27A266a**, swiftlang
  ...
  (`lane-gate`, `ota-stage`, `preota-subset`, `run-instrument`, `ui-bundle-batch`,
  `testflight-stage`, and `run-sweep`'s toolchain default — its `EXPECTED_OS` era pin
  is deliberately untouched, #343's comparability armor). **This is also the Xcode an
  App Store submission requires (#166).** *(History: beta6 was the standard from
  ...
  **⟵ 2026-09-09/10 (#441) — TWO CORRECTIONS AND ONE DECISION.** (1) ~~Beta5
  ...
  `DEVELOPER_DIR` default in `scripts/mac/` moved in that commit; this note is the
  record of why.**
  ...
  fails with errors that read exactly like missing iOS 27 API ("cannot find type
  'IntentExecutionTargets'", "'systemExtraLargePortrait' is unavailable in iOS") — a
  Release simulator build lost ten minutes to SDK-interface archaeology before the log's
  first line (`/Applications/Xcode.app`) settled it. Before diagnosing any "missing iOS 27
  symbol", `grep -m1 '/Applications/Xcode' <log>`.** So the `DEVELOPER_DIR` export is
  mandatory on every hand-rolled call, and for a sharper reason than this line used to
  give (re-point needs sudo; still no urgency — the scripts set their own default). Sim runtimes kept: **iOS 27.0 (24A434, RC — the DEFAULT
