...
**168b — picker does not recover within a session after a cold-offline launch.** Reported as the #163 Gap-1 finding and root-caused here. The picker populates only from the shared `SkillsStore`. On a cold launch with no connectivity the store is empty, the field correctly degrades to free text (degrade-don't-block, as specced) — but nothing re-attempts the fetch when connectivity returns, so the field stays degraded for the whole app session unless the user happens to open the Skills browser and populate the store by hand. This is a SPEC gap, not an implementation error: D5 specified the degrade and never specified recovery (same lifecycle-omission class as 156a's include_disabled miss — a state was specified without its transition out). Fix options: have the picker trigger a `SkillsStore` refresh on appear when its list is empty, or observe connectivity and re-fetch. Prefer on-appear-if-empty; it is cheapest and matches the fetch-on-appear posture the rest of the lane uses.
...
Scope: one small lane, `TaskSkillsPicker.swift` only. 168a is a few lines (a toggle-back button + guard). 168b is on-appear refresh wiring. The design note rides along. No API changes, no other files. Swift 6.2 strict-concurrency conventions apply.
