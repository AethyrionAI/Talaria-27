...
> **✅ LANE 340-U CODE RESULT (2026-09-04, daytime; PR #429, squash `e20f1210`, merged on Owen's standing merge-on-green; DEBUG OTA build 3254 staged from `e20f1210` for the §05 card). Bars 340-U-A and 340-U-B MET (unit); the MECHANISM of 340-U-C/D/E is built and unit-pinned — the numbers themselves are Owen's device run.**
> - **340-U-A MET — `DeviceActionParsing.detectDue(in:now:)` / `detectDueCandidates`.** `NSDataDetector` first; roll-forward ONLY for clock-only matches (the instrument's own prompt at an evening `now` → tomorrow 16:30 — the evening-run trap Task 0 named); an explicit past date → `nil`; a same-day word already past → `nil`; the bare-clock second pass runs ONLY when the detector matched nothing at all and ONLY over an `at <clock>` frame whose bare-integer form passes a continuation allowlist (`look at 5 documents` → `nil`; `in 20 minutes` / `in 3 hours` / `call table 4` → `nil`); never a value ≤ `now`; earliest future of several, with the reversed-order twin. 33 rows, every `now` constructed, relations asserted; mutations exact (nil-always → the 19 resolving rows; drop the guard → the 6 past-clock rows; bare-integer pass → the 3 manufactured-date rows; restore the `candidates.isEmpty` gate → the 3 twins; drop the allowlist → `look at 5 documents`). **The task review caught the lane's one real defect before it shipped: a REJECTED past date fell through to the second pass — `yesterday at 5pm` → today 17:00, `on September 1 at 9am` → tomorrow 09:00 — the founding wrong-value shape, measured live on three phrasings; the gate is now "the detector matched NOTHING", not "no candidate survived".**
> - **340-U-B MET — the per-turn seam.** `ToolEventRelay.beginTurn(userText:)` (defaulted — the bare call CLEARS, so every instrument's per-trial reset stays byte-identical for #343), `currentTurnUserText` (`@MainActor`; `performCreate` already awaits into the relay), `LocalChatBackend.beginToolTurn(userText: message)` from `send` and `streamTurn` — `message`, the user's own bubble, never `promptText` (verified in production source by the reviewer: `message` is an immutable parameter, `prompt` a different local rewritten one line above the call). Source-witness pinned with a positive control; the witness helper hoisted into `RepoSourceWitness` (two former copies now delegate). The `endConversationToolState()` asymmetry (turn-scoped, not cleared there) is provably unreachable: only the `send`/`streamTurn` session carries tools.
> - **340-U-C's mechanism — the fallback in `performCreate`.** `resolvedDue` = `modelDue ?? (rawDue.isEmpty ? detectDue(userText) : nil)`; the three existing guards run on the result UNCHANGED (the past-due grace can only exempt, never flag — pinned); a populated argument, parseable or not, keeps today's path byte-for-byte (the deleted lines reappear verbatim; pinned). The instrument line gains `source=userText|model|none` and `candidates=N` (decision 2's second clause, caught unbuilt by the final review), both BEFORE the greedy `parsed=` (order pinned). **The due-date battery passes each trial's prompt text (`carriesUserText`) — without it the device run would have measured the fallback OFF: the seam's bare `beginTurn()` clears the text, and every instrument calls the bare form per trial. Caught by Task 2's review; #215's unrouted-cell shape one scope up.** The three `#if DEBUG` reminder-tool copies share the one engine (`.armedDateguide` is literally the other arm of this lane's own A/B). `score-due-omission.py`: parses `source=`/`candidates=` (absent → `legacy`, never `none`), **buckets on "did a date reach the CARD" — the old argument-denominated `bucket()` made the bar unmeetable (a fallback-filled trial has `raw=""` and read `omitted`); asserted a no-op on every pre-existing archive**, prints per-cell `populated-future by SOURCE` with explicit zeros (`userText=0/N` on the arm), a `candidates≥2` count per cell, and scores `already-past` for userText rows by parsing `parsed=` (U+202F normalised; C-locale `%b`/`%p` — a non-English device reads cannot-judge). Self-test green.
> - **340-U-D's arm — `armed-nofallback`.** One engine; `ToolEventRelay.disableUserTextDueFallback` (`#if DEBUG`, default false) consulted at exactly ONE point (the fallback term); set per trial by the battery AFTER `beginTurn`; **reset by `beginTurn` AND at the run's end — the task review caught that the first cut left the shared relay (one instance per `AppContainer`) holding `true` after every default run, i.e. the fix silently OFF for every later instrument, warm-up and production turn in that launch**; a tree-wide witness proves every mention sits inside `#if DEBUG`; Release built green before and after. `dueDateBatteryCells` is now `[armed, armedDateguide, armedNofallback]` — the card selects `--cells armed,armed-nofallback`. #416-G: no `nofallback` cell anywhere else.
> - **Gate on `e7928491` (every app and test byte of the branch): PASS on 24A5423a — Swift Testing 3200 (from 3128), XCUITest 18, Release clean.** The merge HEAD `0eceb6e1` adds ONE scorer-only commit (a `bucket()` docstring the wave falsified — the close-out rule — and a labelled overlap line in the whole-archive report; self-test exit 0); no Swift byte differs from the gated commit. Two earlier gate runs on the branch were VOID for reasons outside the diff (the Mac's disk FULL — 124 GB of orphaned DerivedData swept, handoff §30e; then bytes changed by the wave).
> - **Reviews and what they caught (5 task/wave reviews, 3 scoped re-reviews, 1 whole-plan review, all opus/fable):** the fall-through defect and the bare-integer continuation rule (Task 1); the instruments' OFF configuration (Task 2's review, into Task 3); the scorer's argument-denominated bucket and a contradicting heading (Task 3); the flag leak (Task 4); `candidates=` unbuilt, the `route (b)` mislabel, `already-past` un-scoreable for userText (the whole-plan review); the falsified `bucket()` docstring (the wave re-review). **Task 0's own fact (b) — a "fixed waking-hours map" — was FALSIFIED by Task 1 when a row rotted between two runs: the detector's AM/PM choice for an ambiguous bare clock is a 12-hour next occurrence relative to the REAL clock (corrected above, `b47c1d4b`).**
> - **Parked minors, rulings in the lane ledger (`.superpowers/sdd/2026-09-04-340-due-date-from-user-words/progress.md`):** the whole-archive `report()`'s four headline lines no longer partition (a fallback already-past row counts in OMITTED and ALREADY PAST — now labelled; the per-cell view is a true partition and is what the bars read); a mixed legacy/field cell under-reports the `2+` rate; the userText `parsed=` comparison assumes one wall clock (host vs device zone; a DST fall-back hour); `LINE_RE`'s `parsed` group is lazy under the anchor (the comments say greedy — equivalent); `detectDue` is now test-only (a one-line reduction of the list form, pinned equal); the past-calendar-date twin can phantom-red two days a year; `BareClockWiringTests` keeps a latent yield-counted poll (would hang on the first row constructing `NSDataDetector`); `todayAt` force-unwrap on a DST-gap hour; three exact-source-text pins; the `#if DEBUG` scanner over-accepts `||`.
> - **What the device run can and cannot say (the whole-plan review's Q6):** the single instrument prompt carries an explicit meridiem, so 340-U-C measures the INTEGRATION (seam → fallback → card → `source=`) and the evening roll-forward — never the bare-clock second pass or the ambiguous-clock reading, which stay sim-proven until 340-F's diversity cell. 340-U-E is hand-scored from the transcript + the instrument line, per plan.
> - **Owen's side (runbook §05, build 3254 DEBUG):** `run-instrument.sh --device whoGoesThere --instrument due-date --cells armed,armed-nofallback --trials 40` (phone unlocked, Verbose ON, thermal read before/after; ~15 min), same-day `log collect`, `score-due-omission.py --start/--end` (the #416-G window). Stop rule: `populated-future` on `armed` < 34/40 is a MISSED bar and the residual trials are listed verbatim — the consequence is Owen's. Then lane 340-F (ruled above) → OTA → the diversity card.
...
> **📝 2026-09-04 ~17:40 — LANE 425 IN FLIGHT (implemented at `306eafeb` on `.worktrees/425-shelf-local-rows`; review clean on the mechanism, one fix round for a cancellation re-throw).** Two things the review established that the fix paragraph above did not say: **(1) the behaviour of `ChatStore.loadSessions` CHANGES without its bytes changing** — with the router no longer throwing, a host-unreachable load takes the SUCCESS path and the degraded list (local rows + dimmed stubs) is cached under the 15 s snapshot TTL, where the old throw left the timestamp untouched so the next unforced load retried at once. Judged acceptable, arguably an improvement: this entry's own log showed three failed loads in ten seconds each paying a full timeout, the cache collapses those, and every mutation of the list forces a reload; the comment at that catch is corrected in the lane (comment-only). A CANCELLED load (a sheet dismissed mid-fetch on a slow but reachable host) must NOT take that path — the fix round re-throws `CancellationError`/`URLError.cancelled` so the last good list survives. **(2) Two pre-existing limits, not fixed here:** with two profiles configured and one reachable, the partial success records the partial snapshot and `recordRemoteSessionStubs` REPLACES, so the unreachable host's stubs are deleted and a later total failure has none to serve for it; and the search corpus (`lastLoadedSessions`) carries stubs whose fallback summary omits `isUnresumable`, so a stub hit taken through that fallback is tappable.

> **✅ LANE 425 RESULT (2026-09-04 evening; PR #430, squash `5b65f449`, merged on Owen's standing merge-on-green; DEBUG OTA build 3257 staged from it). Bars 425-A, 425-B, 425-C MET; 425-GATE MET — `GATE: PASS on 24A5423a` on the final bytes `34ff18e4` (Swift Testing 3207 from 3200, XCUITest 18, Release clean).**
> - **The fix, one router branch.** `ChatBackendRouter.listSessions()` catches the host-list failure and returns the local rows plus the remote stubs marked unresumable with `hostUnreachableReason` (*"Host unreachable — reconnect to open"* — a second constant, because unpaired ≠ timed out and that row is where the user learns which); the host error is logged at notice level (`listSessions: no configured host answered — <error>; serving N local row(s) + M dimmed host row(s) from the last snapshot`); `recordRemoteSessions` is NOT called on that path (`recordRemoteSessionStubs` REPLACES — calling it would have wiped the stub table). **Cancellation is re-thrown** (`CancellationError` / `URLError.cancelled`) at the top of the catch so a sheet dismissed mid-fetch on a slow but reachable host keeps the last good list. A reachable host keeps the merge byte-for-byte; the not-configured branch is byte-identical (source-pinned). No host, timeout, or health-poll change; no wait widened.
> - **RED-first, mutations exact.** `LocalSessionHistoryTests` 37 → 41 rows: 425-A (a throwing stub host → every local row + every stub, unresumable, no throw; mutation: restore the rethrow → the row); 425-B (the not-configured branch's source pin; mutation: reformat it → only the pin); 425-C (a reachable host records the snapshot; the failure path does not; mutation: record on failure → the failure row); the fix round's two cancellation rows (mutation: drop the re-throw → exactly those two), the empty-host-list-is-success row, the zero-rows-zero-stubs honest-empty row. App target built Debug throughout.
> - **What the review established beyond the bars** (opus review + sonnet scoped re-review): `SessionsHermesClient.listSessions` throws only when NO host answered — a partial success and an EMPTY list are successes and never reach the catch; the stub provider is not gated on configuration; a dimmed row is `.disabled(true)` and `selectSession` guards `isUnresumable`, so "reconnect to open" is TRUE; the Spotlight donation filters to resumable rows (nothing unopenable is written back); the log line can carry a host error body but never a credential. **A behaviour change without a byte change:** `ChatStore.loadSessions` now takes its success path on a host-unreachable load and caches the degraded list under the 15 s snapshot TTL — judged acceptable (the entry's own log showed three failed loads in ten seconds each paying a full timeout; every list mutation forces a reload); the comment at that catch carries a dated note. **The lane's one process error, disclosed by the implementer:** a bare `git checkout --` discarded the uncommitted fix once; rebuilt, and the final GREEN + build ran on the committed bytes.
> - **Known limits, pre-existing, recorded above and not fixed here:** the multi-profile partial-success snapshot replacement; the search corpus's fallback summary omitting `isUnresumable`.
> - **Owen's side:** install build 3257; the §01 runbook card is a one-minute confirm — Tailscale OFF on the phone, open the shelf: every local chat still listed, host rows dimmed with the reconnect reason; Tailscale ON, reopen: the host rows return.

> **🟡 DEVICE RESULT 2026-09-04 ~18:55 (Owen, build 3257, the §01 card): the MECHANISM PASSES, the EXPERIENCE FAILS — filed as 425-D.** Tailscale OFF, Local brain selected, force-quit → relaunch → the shelf was EMPTY … and then, about the time it took to type the report, the rows appeared: Local and PCC normal, Hermes rows greyed and unselectable — exactly the dimmed-stub shape 425-A promised. **What the delay is:** `ChatScreen.refreshSessions` awaits `chatStore.loadSessions`, which awaits the router's MERGED list, which awaits the host list — and with the host unreachable that is the request timeout (this afternoon's log: launch 15:15:48 → the first `loadSessions: FAILED` at 15:16:08, ~20 s). The local rows were in hand from the first millisecond and were held hostage by the host's timeout. **425-D (proposed, Owen's go):** the drawer paints the LOCAL rows first — `loadSessions` publishes the router's local half (local rows + dimmed stubs) as an interim list before awaiting the host half, and repaints with the merged list when it resolves; a reachable host's final list stays byte-identical; the interim paint must never flash a WRONG state (dimmed stubs that then un-dim is the honest sequence). Bars: 425-D1 — with the host unreachable the shelf shows every local row within ~1 s of opening (unit: the interim callback fires before the host call completes; source witness on the drawer); 425-D2 — with a reachable host the final list is unchanged (existing rows green); 425-GATE. The §01 card's PASS wording is re-cut with it.
...
> **⟵ PLAN WRITTEN 2026-09-04 (night, the audit-planning session): `planning/superpowers/plans/2026-09-04-426-transplant-prehistory-on-the-wire.md` — bars 426-A…GATE drafted there, to be pasted into this entry before Task 0 (b). **Task 0 (a) is ALREADY MEASURED, read-only on the Mac gateway:** the host's `agent.turn_context … history=N` line counts OUR body's `conversation_history` (`agent/turn_context.py:797`, checkout `71f8c60f6a`, listener 17:51 — no drift), and the first post-transplant turn reads `history=2` on 08-22/08-23/08-25 (four sessions) and `history=0` on 08-26 17:59/18:01 (the day `faa725df` landed) — the regression is measured on the wire, not inferred. One anomaly is assigned to the lane, not smoothed: the 08-31 session's first turn (`20:18:55 what is the capital of France?`) reads `history=2`; the adjacent `aiohttp.access` User-Agent names the client. Four decisions in the ballot (ship primer+ack verbatim; sim→Mac wire measurement; the planted fact; no new client log line). "Plan owed" is discharged; "bars pre-register when the lane opens" stands; the ballot is Owen's morning; the lane is Saturday's, in harm order #428 → #427 → #426 → #429. Index: `planning/2026-09-04-audit-p1-plans.md`.**

> **⟵ ✅ BALLOT RULED 2026-09-04 (same night, AskUserQuestion): 1 = the primer AND its acknowledgment ride the wire VERBATIM, as the host stored them · 2 = the RED/GREEN wire number is measured from the SIMULATOR against the Mac gateway (the runs plane works on the sim; the host's `history=N` is the count) · 3 = the experience bar's planted fact is OWEN'S OWN — a real, new detail he has never told the agent — and it is deliberately NOT recorded in this repo (public; a real person's name and street stay off it): it lives on the private Device Runbook card only, and the RESULT block records PASS/FAIL · 4 = no new client-side `history=N` log line (the host already counts our body). The synthetic `Dr Patel on Lamar` strings in the plan's TEST fixtures are test data and stay.**

### 🎯 BARS 426-A … 426-GATE — pre-registered 2026-09-04 21:48, BEFORE ANY CODE (verbatim from `planning/superpowers/plans/2026-09-04-426-transplant-prehistory-on-the-wire.md` §Bars; the lane files a RESULT block against these and nothing else; a missed bar is a falsification, never a redefinition)
...
- **426-E — the wire, measured on the host (sim → Mac gateway; then device for the experience half).** (i) CURRENT build (RED): a new local thread with the planted fact, switch to the Mac Hermes profile (transplant fires), one turn → `agent.log`'s `agent.turn_context` line for that turn reads `history=0`. (ii) FIX build (GREEN): same recipe → `history=2` on the first real turn, `history=4` on the second. (iii) Experience, on the phone, Owen's evening: the LOCAL thread states Owen's own planted fact (ruled 09-04 — his new dentist, name and street, supplied on the runbook card and never written into this repo), the thread is switched to Hermes, "hello", then "who is my dentist?" — the reply must name BOTH the dentist and the street. Both numbers carry the app build and the host commit/version; the RESULT block records PASS/FAIL only.
...
> **⟵ ✅ BALLOT RULED 2026-09-04 (same night, AskUserQuestion — Owen took every recommended arm): 1 = a late verdict for a departed thread is DROPPED with one `.notice` line, never redirected into A's cache (the run wrote its turn into A's server session; reopening A fetches it — #321's recorded fact) · 2 = walk-away CANCELS `reconcileInFlight` AND bumps `recoveryGeneration` (the token still catches a read past its cancellation check) · 3 = Stop gets the same protection — `abandonReconcileWindowOnStop` cancels + bumps, so the window #321 closes stays closed against an in-flight read · 4 = `Conversation.id` identifies the thread in the token (the identity `PendingRunRecord.conversationID` and the cold-load guard already use). The plan carries the same block; nothing in it is still a default.**

### 🎯 BARS 427-A … 427-GATE — pre-registered 2026-09-04 21:48, BEFORE ANY CODE (verbatim from `planning/superpowers/plans/2026-09-04-427-recovery-ownership-token.md` §Bars; the lane files a RESULT block against these and nothing else; a missed bar is a falsification, never a redefinition)
...
> **⟵ PLAN WRITTEN 2026-09-04 (night, the audit-planning session): `planning/superpowers/plans/2026-09-04-428-voice-restart-outlives-shutdown.md` — bars 428-A…D + GATE drafted there. Shape: a `captureGeneration` inside the actor that `stop()` bumps and `start()` re-checks after its awaits and immediately before the install/start stretch (`CaptureError.superseded`, one `.notice` line `capture start ABANDONED …`); teardown cancels AND joins `restartTask` (bounded, decision 1); a superseded restart never repaints an ended session; two seams (`NativeVoiceCapturing`, `SpeechAnalysisAssembling`) make both layers unit-testable, the service layer driven through the REAL `routeChangeNotification` path. Task 0 has a HARDWARE half (five route-change-then-End trials on the current build, Owen's ~10 min — does the ordering reproduce?) and a sim half (engine start / mic preflight / the 750 ms cooldown). Four decisions in the ballot (3 s join bound; silent supersession; Task 0 on hardware first; the assembler seam). FIRST in harm order #428 → #427 → #426 → #429. "Plan owed" is discharged; "bars pre-register when the lane opens" stands; the ballot is Owen's morning; the lane is Saturday's, in harm order #428 → #427 → #426 → #429. Index: `planning/2026-09-04-audit-p1-plans.md`.**

> **⟵ ✅ BALLOT RULED 2026-09-04 (same night, AskUserQuestion — Owen took every recommended arm): 1 = 3 s BOUNDED join in teardown (a wedged analyzer prep must not turn End into a hang; the capture generation covers the straggler past the bound) · 2 = a superseded restart is SILENT (one `.notice` line, no `.failed` repaint) · 3 = Task 0 runs on HARDWARE FIRST — five AirPods-connect-then-End trials on the CURRENT build with the existing HOT/COLD instrument, filed as 428-T0a before the lane opens; the card lands on the Device Runbook §04 · 4 = the controller seam is an injectable assembler behind `SpeechAnalysisAssembling` (production code, no DEBUG hooks). The plan carries the same block; nothing in it is still a default.**

### 🎯 BARS 428-A … 428-GATE — pre-registered 2026-09-04 21:48, BEFORE ANY CODE (verbatim from `planning/superpowers/plans/2026-09-04-428-voice-restart-outlives-shutdown.md` §Bars; the lane files a RESULT block against these and nothing else; a missed bar is a falsification, never a redefinition)
...
> **⟵ PLAN WRITTEN 2026-09-04 (night, the audit-planning session): `planning/superpowers/plans/2026-09-04-429-markdown-images-tap-to-load.md` — bars 429-A…E + P + GATE drafted there. Shape (Owen's default, tap to load): ONE `RemoteImageView` becomes the only `AsyncImage` site, gated by a per-launch, URL-keyed `RemoteImageConsent` (keyed by URL because `MarkdownSegment.image`'s id is minted per parse and a streaming transcript re-parses every delta); a placeholder naming the host; the bar is a hosted render of the real view with a counting `URLProtocol` and a LIVE control arm (the #259 standard). Task 0 measures the one premise — does the hosted render issue `AsyncImage`'s request through a registered protocol in the test host? — and picks design A (gate `AsyncImage`) or B (an injectable loader). Five decisions in the ballot (tap-to-load vs trusted-origins vs block; per-launch scope; user bubbles same rule; the copy; the privacy sentence rides this lane, held for Owen's read). "Plan owed" is discharged; "bars pre-register when the lane opens" stands; the ballot is Owen's morning; the lane is Saturday's, in harm order #428 → #427 → #426 → #429. Index: `planning/2026-09-04-audit-p1-plans.md`.**

> **⟵ ✅ BALLOT RULED 2026-09-04 (same night, AskUserQuestion — Owen took every recommended arm): 1 = TAP TO LOAD (a placeholder naming the host; nothing fetched until tapped; no allowlist) · 2 = approval lasts the LAUNCH, keyed by absolute URL, never persisted · 3 = user-authored bubbles follow the SAME rule · 5 = the privacy page's sentence ships IN THIS LANE, the PR HELD for Owen's read of the exact wording (`docs/` publishes on merge). Decision 4 (placeholder copy) was not balloted — the plan's default stands (`IMAGE · <host>` / `Tap to load` / alt text) unless Owen overrules it at the RESULT read. Nothing in the plan is still a default.**

### 🎯 BARS 429-A … 429-GATE — pre-registered 2026-09-04 21:48, BEFORE ANY CODE (verbatim from `planning/superpowers/plans/2026-09-04-429-markdown-images-tap-to-load.md` §Bars; the lane files a RESULT block against these and nothing else; a missed bar is a falsification, never a redefinition)
...
> **Source:** audit A6. **Verified:** `TalariaShare/ShareViewController.swift:212-224` gates on `StageableTypeCatalog.isStageable(fileName:)` + `data.count <= remainingBytes` against `ShareInboxCore.swift:147` `defaultMaxEnvelopeBytes = 20_000_000`; the app's `PendingAttachment.swift:180` `guard data.count <= stagingCap(forMimeType:)` with `maxFileSize = 350 * 1024` (`:36`) and `maxPDFFileSize = 10 * 1024 * 1024` (`:41`) — **the non-PDF band is a 58× window**; `ShareInboxDrainer.swift:53` logs `skipped unconvertible item` and `:57` removes the envelope; the caller (`AppContainer.swift:1659-1665`) logs only `envelopeCount` and seeds the composer — no per-item result reaches the user. **Tracker:** the CLASS is filed, this instance is not — archive #123 (`:45872-45877`) states the intended contract in its own words ("visible reasons instead of vanishing at drain time") and notes the drain re-uses `PendingAttachment.file(at:)` without reconciling its tighter caps; #180-E (`OPEN_ITEMS.md:3290`) fixed only the 20 MB label arithmetic. **Tests pin the silent skip as correct:** `ShareInboxDrainTests.drainSkipsUnsupportedBlobAndKeepsTheRest:81`, `drainWithNothingConvertibleReturnsNilButConsumes:103`; no size-band case. **Fix shape:** share the size policy with the type catalog (per-MIME caps in `StageableTypeCatalog`), refuse over-cap items in the extension BEFORE Send with a visible reason, and surface conversion failures at drain instead of consuming them; bars at both size boundaries, corrupt PDFs, and the aggregate limit.
...
> **Source:** audit A9. **Cross-reference:** #157 (archive `:5096`, CLOSED 2026-07-22) reproduced the WebRTC BSD-3-Clause notice in `THIRD_PARTY_LICENSES.md` and explicitly deferred an in-app acknowledgements screen as "NOT blocking submission … should not be built speculatively"; no tracker entry mentions OFL, Chakra Petch, Space Grotesk or JetBrains Mono in a licensing sense; `project.yml:107` (resource packaging) and `:225` (the font declarations) ship the nine files. The OFL's redistribution terms address including the copyright and license notice with redistributed copies; the audit's font-metadata read found the OFL NAMED and linked, not reproduced. **Shape:** add the three OFL notices (verbatim texts from the publishers) to `THIRD_PARTY_LICENSES.md`, bundle a notices resource (or a Settings → About → Licenses surface — the deferred #157 follow-on, now with a reason) and verify the final archive contains it. A distribution-readiness gap, not a legal determination about every embedded binary field.

## 435. 📄 WEATHER OUTPUT HAS NO ATTRIBUTION MECHANISM — the weather tool returns conditions/forecasts as ordinary model input and nothing deterministic attributes Apple Weather; `THIRD_PARTY_LICENSES.md:205` groups WeatherKit under "no attribution obligation", conflating using the framework with DISPLAYING its data — **FILED 2026-09-04 (audit A10, P2; never considered before — #212's WeatherKit history is entitlement bugs only). Pre-submission; Owen's read of Apple's requirement.**

> **Source:** audit A10; Apple's WeatherKit attribution requirements (`https://developer.apple.com/weatherkit/#attribution-requirements`) describe attribution for displayed weather data and a separate rule for transformed, value-added products (which also requires attribution). **Cross-reference:** `DeviceReadTools.swift:462` (the lookup and returned data); no attribution surface in the inspected code; a model that mentions Apple occasionally does not establish that every displayed result carries it. **Shape:** a deterministic attribution tied to weather-derived output (a chip/footnote on the reply that drew on the weather tool, and the voice-only presentation), correct the blanket statement in the notice document, and verify the shipped experience against Apple's text rather than delegating compliance to generated prose.
