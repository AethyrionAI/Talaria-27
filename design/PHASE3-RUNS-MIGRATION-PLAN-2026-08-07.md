...
> **2026-08-07 — ROUTED: Owen said "begin on phase 3" (Q2 answered; the other §5
> questions stand as recommended/pending, none blocking 3A). Slice 3A's lane is
> tracker #283. The 3A-0 blocking probe RAN the same day, before any code — N4
> and N9 both carry dated ANSWERED notes in §1.6 below, and the two bars the
...
| **N4** | **`/v1/runs` does not read session history from the DB.** `_handle_runs` builds `conversation_history` from the request body or `previous_response_id` (`:6329-6360`), where the chat handlers call `_conversation_history_for_session` (`:3584`, `:3747`). Whether a run carrying an existing `session_id` **writes** its turn back into the SessionDB row that `/api/sessions/{id}/messages` reads is **UNVERIFIED** — `session_id` is passed to `_create_agent` (`:6022`) and `task_id = session_id or run_id`, but nothing in the runs handler touches the DB. **✅ ANSWERED 2026-08-07 (3A-0 wire probe, live Mac gateway, session `api_1786118893_f3250a12`): a SPLIT — runs WRITE the row but never READ it.** Write ✓: both user and assistant rows landed verbatim in the `/messages` row (shared path: `_create_agent(session_db=…)` → `run_conversation` → `turn_finalizer:410` `_persist_session` → `_flush_messages_to_session_db`). Read ✗: a second run on the same `session_id` had no access to the first's marker and answered from LONG-TERM MEMORY (a 2-day-old probe leftover) — continuity failures hallucinate plausibly, they don't fail loud. `previous_response_id` is no escape: only `/v1/responses` handlers `put()` into the response store (`:4557`, `:5352`); runs never store. | CODE-READ, then **PROVEN 2026-08-07** | **CARRIED → #283.** History/`openSession`/fork/#190 survive (write half); the app must supply per-turn context (send local thread, or pre-fetch `/messages` — decided inside 3A; bar 3A-G pins it). |
...
| **N9** | **Attachment shape on runs is unverified.** `_handle_runs` takes `input` as a string or a message array and extracts `raw_input[-1]["content"]` without flattening multi-part content (`:6318-6320`); only *history* entries get the part-flattening treatment (`:6360-6370`). Our chat body sends attachments through `ChatTurnBody.make(...)` (`SessionsHermesClient.swift:285`). **✅ ANSWERED 2026-08-07 (3A-0 wire probe): the app's exact parts-array shape is DEAD on `/v1/runs`** — `400 "No user message found in input"`, both part orders (a content part has no `content` key for `:6320` to find). **The message-array wrap `input=[{role:user, content:[parts]}]` works end-to-end**: 202, run completed, agent answered the red-PNG fixture's color correctly (chat-plane control identical). Caveat: runs skip `_normalize_multimodal_content` (`:550`) — parts reach the agent unvalidated, so the client keeps its own data:image-only/size discipline. Text-only turns (plain-string `input`) are unaffected. | CODE-READ, then **PROVEN 2026-08-07** | **CARRIED → #283** (bar 3A-H: message-array wrap in the client; the parts-array shape unreachable from production code on the runs path). |
...
- **Session lifecycle and history** — `POST /api/sessions`, `GET /api/sessions/{id}/messages`,
  ...
  **[2026-08-07 — N4 answered as a split neither branch anticipated: persistence ✓ (the
  server transcript stays authoritative and current, this bullet's "stays" holds), read ✗
  (the app must still supply per-turn context — from its local thread or a pre-turn
  `/messages` fetch). See §1.6 N4 and #283.]**
...
- **3A-0 (probe, blocking):** N4 answered — does a `/v1/runs` call carrying an existing
  ...
  **✅ RAN 2026-08-07, both answered before any code — see §1.6 N4/N9 (dated notes) and #283,
  which owns the refined bar set including the two bars the answers forced (3A-G history
  continuity, 3A-H attachment reshape).**
...
- **N4 unanswered is the big one.** If runs do not persist into SessionDB, slice 3A grows a
  ...
  **[2026-08-07 — removed: persistence holds, so the transcript story is unchanged; the slice
  grows only the per-turn history-supply mechanism (the smaller half of this risk). The sharp
  residual is that a missing/short history does not fail loud — the agent answers from
  long-term memory instead (probe evidence in #283) — which is why 3A-G asserts content, not
  mere non-emptiness.]**
