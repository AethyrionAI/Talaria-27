...
extension LocalChatBackend {
    ...
    enum ActionBatteryCell: String, CaseIterable {
        ...
        ///
        /// **⚠️ CORRECTED 2026-09-01 (#340-PROMOTE): the first paragraph above
        /// is stale.** Production's `due` guide no longer offers *"…or empty for
        /// no due date"* and no longer stays silent on bare times — #340's
        /// promotion put the bareclock text there. So `armed` is not this arm's
        /// control any more, and an `armed` vs `armed-dateguide` run now
    ...
    ///
    /// **⚠️ 2026-09-01 (#340-PROMOTE): `.armed` IS NO LONGER A CONTROL HERE.**
    /// The promotion put 340-H5′'s winning bareclock guide into production, so
    /// this pair now runs treatment against treatment. The retired
    /// `bareClockBatteryCells` list that used to sit below this one is gone with
    /// the cell it named — and it had **zero call sites** even before that: the
