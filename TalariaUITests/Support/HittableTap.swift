...
enum HittableTap {
    ...
    static func diagnosticLines(
        ...
        lines.append("""
            ...
            buttons=\(app.buttons.count) window=\(describe(app.windows.firstMatch.frame)) \
            scroll=\(describe(app.scrollViews.firstMatch.frame))
            """)
        ...
        // When there are more than the cap, keep the DEEPEST: the dump is
        // depth-first, so the tail is what is drawn on top — and what is on
        // top is the question being asked.
        let shown = covering.suffix(maximumElementsUnderPoint)
        if shown.isEmpty {
        ...
        for (index, row) in shown.enumerated() {
            lines.append("""
                \(activityPrefix)under[\(index + 1)/\(shown.count)] \(row.type) \
                id=\(row.identifier) frame=\(describe(row.frame))
                """)
        }
        if covering.count > shown.count {
            lines.append("\(activityPrefix)under truncated total=\(covering.count) shown=\(shown.count)")
        }
...
extension XCTestCase {
    ...
    ///
    /// Existence only, unchanged from both originals: callers use a `nil`
    /// return as "chat never came up", and every one of those call sites is a
    /// landing check rather than a tap. Where a tap DOES follow, hittability
    /// is polled at the tap itself — `tapWhenHittable` — so this wait keeps
    /// its meaning and no budget moves.
    @MainActor
