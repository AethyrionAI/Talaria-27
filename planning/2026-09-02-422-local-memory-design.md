...
> **⟵ SUPERSEDED 2026-09-03 (Owen's ruling after bar 422-R's measurement, lane M2): THE EMBEDDER IS DELETED FROM THE SHAPE — retrieval is LEXICAL-ONLY.** On the 104-turn / 87-query labelled corpus the hybrid scored p@1 0.827 · false-admit 0.750 · top-3 0.920, and lexical-only scored 0.853 · 0.750 · 0.973 — mutation M-R fired (lexical-only strictly lower on none of the three). With the lexical anchor in place cosine never ADMITS, it only re-ranks, and `NLEmbedding.sentenceEmbedding` re-ranks worse (it scores interrogative form heavily; the 16-turn smoke reading below did not see this). Shipped: relative admission `z = 1.5` + lexical anchor (the top-2 % clause dropped — it re-admitted pure-cosine artifacts) over `LexicalTokenizer.lexicalOverlap` with a light suffix stemmer (a wash on the numbers, kept for robustness). Rows carry text only — no `vector`, no `embedderID`. The false-admit bar was JOINTLY INFEASIBLE with p@1 ≥ 0.80 on the adversarial no-answer class (exhaustive ≈ 15 k-config search: p@1 ≤ 0.560 under fa ≤ 1/12); Owen accepted the miss for the pre-launch shape and a PLAIN no-answer class (20 unrelated questions sharing no content token with any turn) was added — 0/20 by construction under a lexical scorer, kept as a regression guard. Final numbers: **p@1 0.867 (65/75) · top-3 0.960 (72/75) · false-admit plain 0/20 · adversarial 9/12** (= 9/9 of the genuine near-misses). The rest of this section describes the shape that was measured and retired, not the one that shipped.

> **⟵ CORRECTED 2026-09-04 (lane 422-S/T/U).** The "final numbers" above are stale: the 09-03 PM ruling accepted **p@1 0.800 (60/75) · top-3 0.947 (71/75)** as the bar once the tie-break was made total, and every reproduction of the shipped scorer returns exactly that (a standalone copy of the tokenizer + retriever, and the in-suite `422-R:` line). The 65/75 · 72/75 figure predates the total tie-break. **And the scorer has moved since:** bar 422-S added a ~90-word function-word stop list (light verbs, modals, discourse words) after a "can you tell me …" frame was measured at 50/75 · 60/75 on the same corpus — with it the corpus scores **62/75 · 72/75 as written and 62/75 · 72/75 under every frame tried**, plain false-admit 0/20 by construction, adversarial 9/12 unchanged. Two further shape changes ride the same lane: the current conversation's rows are never retrieval candidates (422-T — the hits preamble says "your earlier chats"), and the honesty guard treats a PROMISE to remember as kept by the index on an indexed thread while still correcting a claimed WRITE (422-U; the correction copy is now "No note was saved…"). Tracker #422 carries the bars, the RED/GREEN/mutation record and the gate.
...
> **⟵ CORRECTED 2026-09-03 (lane M3, Task 10's review): with memory ON the phone's effective prompt budget is 6,368 tokens, not 7,168** — `fitsContext` now RESERVES one memory block (800 on the 8,192 window) for the current turn's own prefix, which the accounting for prior turns could not see (one unaccounted ~330-token block per generation, up to ~460 with a just-saved notice, two on a retry leg). So the "5,248 / 6,828 left" arithmetic in the row above is 4,448 / 6,028 with memory on; with memory OFF the row stands unchanged. The condensed-tail estimate (~3,400 verbatim tokens ≈ 40 median exchanges) becomes ~2,600 ≈ 30. Also measured in that lane: the accounting rebuild fires at min(1,500, budget/4) accumulated prefix tokens ≈ every 4–5 retrieving turns — the re-prefill cost of that cadence is UNMEASURED and rides 422-L.
...
- **422-C (EMBEDDER — offline + device).** *Offline, on the sim:* a test constructs `NLContextualEmbedding(language: .english)` first and asserts `NLEmbedding.sentenceEmbedding(for: .english) != nil` — the 6/6 order-dependence, pinned (the test is deleted the day a runtime makes it pass without the warm-up, and says so). With the embedder forced `nil`, retrieval still answers from the lexical scorer (top-1 ≥ 8/12 on Appendix B's corpus). Every row carries `embedderID`; a row whose id ≠ the live embedder's is never scored (RED-first: a mismatched row scored as if comparable). *Device arm:* on `whoGoesThere`, record for both embedders: constructible, `hasAvailableAssets`, `load()` result, `dimension`, per-turn latency (n=20) — with `osVersion` and build in the artifact. **No number in this lane is quoted without this row.**
  **⟵ SUPERSEDED 2026-09-03: bar 422-C is retired with the embedder (Owen's ruling, see §2.3's note) — both embedders DO load on the phone (the 09-02 device arm stands as a fact) and the in-bundle first-call-nil mechanism was measured and corrected; they simply buy nothing on the labelled corpus. Nothing here was falsified; it was made moot.**
- **422-R (RETRIEVAL QUALITY — offline).** A labelled corpus of **≥ 100 user turns and ≥ 40 queries, ≥ 10 of them no-answer queries**, checked into `planning/reports/` with the run. Bars on the hybrid + relative-admission scorer: **precision@1 ≥ 0.80** on answerable queries; **false-admit rate ≤ 0.10** on no-answer queries (an admit is any chunk passing the rule); top-3 recall ≥ 0.90. Reported over the same denominator, per query class. **Mutation M-R:** lexical-only must score strictly lower on ≥ 1 of the three; if it does not, the embedder buys nothing and is **deleted from the shape** — that is a legitimate outcome, recorded as one, not a bar rewritten.
  **⟵ RE-SCOPED BY RULING 2026-09-03 (not rewritten by the lane): mutation M-R FIRED and the pre-registered consequence was taken (embedder deleted). The false-admit clause is jointly infeasible with p@1 ≥ 0.80 on the adversarial no-answer class; Owen accepted the miss and added a PLAIN class whose acceptance number he sets after seeing it. Measured lexical-only + stemmer: p@1 0.867 · top-3 0.960 · fa plain 0/20 · fa adversarial 9/12.**
