...
> **✅ BALLOT RULED 2026-09-04 (Owen, AskUserQuestion, the same night the plan was written):** 1 = **primer AND ack, verbatim** · 2 = **sim → Mac gateway** for the wire number · 3 = **Owen's OWN fact** — a real, new detail about himself that he has never told the agent (his words: "my actual dentist, but he's new so I've not spoken about it"). **The fact itself is deliberately NOT recorded in this repo** (the repo is public; a real person's name and street stay off it) — it lives only on the private Device Runbook card, and the RESULT block records PASS/FAIL, never the fact · 4 = **no new client log line**. Every recommended arm below is now a ruling; the synthetic `Dr Patel on Lamar` strings in the TEST fixtures stay as they are — they are test data, not the device fact.
...
- [ ] `scripts/mac/ota-stage.sh main Debug`; one §01 runbook card for 426-E (iii): local thread stating Owen's own planted fact (on the card only) → switch to Hermes → "hello" → "who is my dentist?" → the reply names the dentist AND the street; PASS/FAIL; same-day `agent.log` read on the host (`history=2` then `history=4`).
