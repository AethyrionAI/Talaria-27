...
| R6 | Store build exported under Xcode-RC from merged `main`, family + manifest verified inside the ipa | ✅ **`~/.talaria-ota/testflight/Talaria27-store-3328.ipa`** (main @ `f86f7901`, build 3328 — carries the #444 hostless-honesty fix; 3324/3313 moved to `retired/`); upload is Owen's |
...
2. ✅ `scripts/mac/testflight-stage.sh` under Xcode-RC → **`~/.talaria-ota/testflight/Talaria27-store-3328.ipa`** (main @ `f86f7901`, build 3328, 1.0.0, minOS 27.0 — includes the #444 fix: a hostless install no longer claims LINKED/CONNECTED on Settings); `UIDeviceFamily [1]` and the empty scene-configuration dict verified inside the ipa; both appexes present. The earlier 3324 (pre-#444) and 3313 (beta6, universal) ipas live in `retired/` — the one loose ipa in the folder is the one to upload.
...
11. **Upload** `~/.talaria-ota/testflight/Talaria27-store-3328.ipa` with Transporter (Mac App Store) signed in as the developer Apple ID. Wait for "processing" to finish (minutes to an hour). Select the build on the version page. Export compliance should not prompt (166d); if it does: exempt only.
