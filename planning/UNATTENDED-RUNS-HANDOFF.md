...
> **48 entries since #335, 2026-08-12; 19 unattended-eligible** — the three new
...
|---|---|---|---|
| #257 tokenCount pre-flight | ~~never built~~ **`tokencount-preflight`** (#335, 2026-08-12) | iPad | runs outside any turn by construction — no generation at all |
| #324-W3 | ~~not built~~ **`fm-asymmetries`** (#335) — all three bands in one run: `tokenCount` 4096-vs-8192, `variant.displayName`, `maximumResponseTokens` throw-vs-truncate | iPad | ⚠️ **beta5 runtimes ONLY** — the variant band is a new-in-beta5 symbol and a beta4 runtime kills the app at dyld launch (#324) |
| #211A | offer-instead-of-act on READ paths | iPad | read-only prompts by construction |
| #210A / #210 residual | ~~never measured~~ **`condensation-fit`** (#335) — one instrument answers both: does ONE forced condensation fit 8,192? | iPad | a trial scores only if ARMED (pre-count measured >8,192); if none arms, the residual stays open |
